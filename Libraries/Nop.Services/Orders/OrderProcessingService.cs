    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual async Task SetOrderStatusAsync(Order order, OrderStatus os, bool notifyCustomer)
        {
                ...
                && notifyCustomer)
            {
                ...
                // added by clement on 26th Apr 2022 - order cancellation notification to Store Owner
                var orderCancelledStoreOwnerNotificationQueuedEmailIds = await _workflowMessageService.SendOrderCancelledStoreOwnerNotificationAsync(order, order.CustomerLanguageId);
                if (orderCancelledStoreOwnerNotificationQueuedEmailIds.Any())
                    await AddOrderNoteAsync(order, $"\"Order cancelled\" email (to Store Owner) has been queued. Queued email identifiers: {string.Join(", ", orderCancelledStoreOwnerNotificationQueuedEmailIds)}.");
            }
        ...
        protected virtual async Task ProcessOrderPaidAsync(Order order)
        {
            ...
            if (order.OrderTotal != decimal.Zero)
            {
                ...
                if (orderPaidCustomerNotificationQueuedEmailIds.Any())
                    await AddOrderNoteAsync(order, $"\"Order marked as paid\" email (to customer) has been queued. Queued email identifiers: {string.Join(", ", orderPaidCustomerNotificationQueuedEmailIds)}.");
                ...
                if (orderPaidStoreOwnerNotificationQueuedEmailIds.Any())
                    await AddOrderNoteAsync(order, $"\"Order marked as paid\" email (to store owner) has been queued. Queued email identifiers: {string.Join(", ", orderPaidStoreOwnerNotificationQueuedEmailIds)}.");
            ...
            switch (order.OrderStatus)
            {
                ...
                case OrderStatus.Cancelled:
                    if (order.ShippingStatus == ShippingStatus.PartiallyShipped ||
                        order.ShippingStatus == ShippingStatus.Shipped ||
                        order.ShippingStatus == ShippingStatus.Delivered)
                        await SetOrderStatusAsync(order, OrderStatus.Cancelled, false);
                    break;
                ...
                case OrderStatus.Refunded:
                    if (order.PaymentStatus == PaymentStatus.Refunded ||
                        ...
                        await SetOrderStatusAsync(order, OrderStatus.Refunded, false);
                    return;
            }
        ...
        public virtual bool CanCancelOrder(Order order)
        {
            ...
            if (order.OrderStatus == OrderStatus.Pending)
            {
                ...
                order.PaymentStatus == PaymentStatus.Voided || order.ShippingStatus == ShippingStatus.PartiallyShipped || order.ShippingStatus == ShippingStatus.Delivered || order.ShippingStatus == ShippingStatus.Shipped)
                    return false;
                else
                    return true;
